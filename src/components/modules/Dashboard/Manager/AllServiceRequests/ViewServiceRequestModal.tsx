...
interface ViewServiceRequestByIdDialogProps {
  open: boolean;
...
export default function ViewServiceRequestByIdModal({
  open,
  onOpenChange,
  serviceRequest,
}: ViewServiceRequestByIdDialogProps) {
  const serviceRequestId = serviceRequest ? String(serviceRequest.id) : "";

  const { data, isLoading, isFetching } = useQuery({
    queryKey: ["service-request-details", serviceRequestId],
                      ...
                      {details.customer ? (
                        <div className="flex items-center gap-4 p-4 rounded-2xl border border-indigo-100 bg-indigo-50/30">
                          ...
                          <div>
                            ...
                            <p className="text-md font-bold dark:text-slate-300 text-slate-600">
